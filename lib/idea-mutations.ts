...
// Same rule as lib/category-mutations.ts: these take userId and perform no
// auth of their own, so they must never be exported from a "use server" file
// (every such export is a publicly callable server action). The thin actions
// in app/(app)/ideas/actions.ts call requireUser() and then delegate here.
